...
// ─── Sacred Strange: Unified parametric golden geometry ───
// Ported from static/sacred-strange.html
// Same algorithmic core as radiant-geometry but with separate entry point

// Default parameter values (screensaver — no interactive controls)
constant float SS_ROTATION_SPEED = 0.3;
constant float SS_COMPLEXITY = 1.0;
constant float SS_PATTERN = 0.05;

constant float SS_PI = 3.14159265359;
constant float SS_TAU = 6.28318530718;
constant float SS_PHI = 1.6180339887;
constant float SS_SQRT3 = 1.7320508;

// ── Rotation matrix ──
static float2x2 ss_rot(float a) {
    float c = cos(a), s = sin(a);
    return float2x2(float2(c, s), float2(-s, c));
}

// ── Golden color palette ──
static float3 ss_gold(float t) {
    float3 a = float3(0.45, 0.32, 0.14);
    float3 b = float3(0.45, 0.35, 0.2);
    float3 c = float3(1.0, 0.8, 0.5);
    float3 d = float3(0.0, 0.1, 0.25);
    return a + b * cos(SS_TAU * (c * t + d));
}

// ── Polar fold ──
static float2 ss_polarFold(float2 p, float n) {
    float angle = atan2(p.y, p.x);
    float sector = SS_TAU / n;
...
// ── SDF: line segment ──
static float ss_sdSegment(float2 p, float2 a, float2 b) {
    float2 pa = p - a;
    float2 ba = b - a;
...
// ── SDF: circle ring ──
static float ss_sdRing(float2 p, float r) {
    return abs(length(p) - r);
}

// ── SDF: regular polygon outline ──
static float ss_sdPolygon(float2 p, float r, float n) {
    float angle = atan2(p.y, p.x);
    float sector = SS_TAU / n;
...
static float3 ss_glowLine(float d, float3 coreColor, float3 bloomColor, float lineWidth, float bloomWidth) {
...
// ── Star motif ──
static float ss_starMotif(float2 p, float symmetry, float breathe, float innerRatio, float petalInfluence) {
    float d = 1e9;
    float doubleSym = symmetry * 2.0;
    float2 fp = ss_polarFold(p, doubleSym);

    float2 a1 = float2(0.5 * breathe, 0.0);
    float2 b1 = float2(mix(0.35, 0.32, innerRatio) * breathe, mix(0.15, 0.13, innerRatio) * breathe);
    d = min(d, ss_sdSegment(fp, a1, b1));

    float innerR = mix(0.22, 0.18, innerRatio);
    float2 a2 = b1;
    float2 b2 = float2(innerR * breathe, 0.0);
    d = min(d, ss_sdSegment(fp, a2, b2));

    float2 a3 = b2;
    float2 b3 = float2(mix(0.12, 0.10, innerRatio) * breathe, mix(0.07, 0.06, innerRatio) * breathe);
    d = min(d, ss_sdSegment(fp, a3, b3));

    float2 a4 = b3;
    float2 b4 = float2(0.0, 0.0);
    d = min(d, ss_sdSegment(fp, a4, b4));

    d = min(d, ss_sdRing(p, 0.5 * breathe));
    d = min(d, ss_sdRing(p, innerR * breathe));

    if (petalInfluence > 0.01) {
        float petalR = 0.5 * breathe * 0.35 / SS_PHI;
...
// ── Hex-tile a motif ──
static float ss_hexTileMotif(float2 p, float scale, float symmetry, float breathe, float innerRatio, float petalInfl) {
    p *= scale;
    float2 s = float2(1.0, SS_SQRT3);
    float2 h = s * 0.5;
    ...
    float d = ss_starMotif(gUV, symmetry, breathe, innerRatio, petalInfl);
    return d / scale;
}

// ── Central motif ──
static float ss_centralMotif(float2 p, float scale, float symmetry, float breathe, float innerRatio, float petalInfl) {
    p *= scale;
    float d = ss_starMotif(p, symmetry, breathe, innerRatio, petalInfl);
    return d / scale;
}

// ── Unified geometry layer ──
static float ss_geometryLayer(float2 p, float scale, float symmetry, float breathe, float innerRatio, float petalInfl, float tilingMix) {
    float tiledScale = scale;
    float centralScale = scale * 0.35;
    float effectiveScale = mix(tiledScale, centralScale, tilingMix);

    if (tilingMix < 0.01) {
        return ss_hexTileMotif(p, effectiveScale, symmetry, breathe, innerRatio, petalInfl);
    } else if (tilingMix > 0.99) {
        return ss_centralMotif(p, effectiveScale, symmetry, breathe, innerRatio, petalInfl);
    } else {
        float dTiled = ss_hexTileMotif(p, mix(tiledScale, tiledScale * 0.6, tilingMix), symmetry, breathe, innerRatio, petalInfl);
        float dCentral = ss_centralMotif(p, mix(centralScale * 1.5, centralScale, tilingMix), symmetry, breathe, innerRatio, petalInfl);
        return mix(dTiled, dCentral, tilingMix);
    }
}

// ── Golden spiral SDF ──
static float ss_goldenSpiral(float2 uv, float t, float rotSpd) {
    float r = length(uv);
    float a = atan2(uv.y, uv.x);
    float spiralPhase = log(max(r, 0.001)) / log(SS_PHI) * SS_PI * 0.5;
    float v1 = a - spiralPhase + t * rotSpd * 0.2 + SS_PI;
    float spiralD = abs(v1 - SS_TAU * floor(v1 / SS_TAU) - SS_PI);
    float v2 = v1 + SS_PI;
    spiralD = min(spiralD, abs(v2 - SS_TAU * floor(v2 / SS_TAU) - SS_PI));
    float fade = smoothstep(0.0, 0.05, r) * smoothstep(0.5, 0.35, r);
    return spiralD * fade + (1.0 - fade);
}

fragment float4 fs_sacred_strange(VSOut in [[stage_in]],
                                  constant CommonUniforms& u [[buffer(0)]]) {
    float2 uv = (in.pos.xy - u.resolution * 0.5) / min(u.resolution.x, u.resolution.y);
    float t = u.time;
    float rotSpeed = SS_ROTATION_SPEED;
    float complexity = SS_COMPLEXITY;
    float pat = clamp(SS_PATTERN, 0.0, 1.0);
    float r = length(uv);
    ...
    // LAYER 1
    {
        ...
        float2 p1 = ss_rot(rot1) * uv;
        float scale1 = mix(2.5, 1.8, pat) * complexity;
        float d1 = ss_geometryLayer(p1, scale1, sym1, breathe, 0.0, petalInfluence * 0.3, tilingMix);
        col += ss_glowLine(d1, medGold, dimGold * 1.5, 0.002, 0.016);
    }
    ...
    if (numLayers > 2.0) {
        ...
        float2 p2 = ss_rot(rot2) * uv;
        float scale2 = mix(3.5, 2.2, pat) * complexity;
        float innerR2 = mix(0.3, 0.6, pat);
        float d2 = ss_geometryLayer(p2, scale2, sym2, breathe, innerR2, petalInfluence * 0.5, tilingMix);
        float layer2alpha = min(numLayers - 2.0, 1.0);
        col += ss_glowLine(d2, brightGold * 0.8, dimGold * 1.2, 0.0015, 0.014) * layer2alpha;
    }
    ...
    if (numLayers > 3.0) {
        ...
        float2 p3 = ss_rot(rot3) * uv;
        float scale3 = mix(4.0, 2.5, pat) * complexity;
        float d3 = ss_geometryLayer(p3, scale3, sym3, breathe * (1.0 + 0.01 * sin(t * 0.5 + 2.0)), 0.5, petalInfluence * 0.7, tilingMix);
        float layer3alpha = min(numLayers - 3.0, 1.0);
        col += ss_glowLine(d3, coreGlow * 0.6, medGold * 0.7, 0.0012, 0.012) * layer3alpha;
    }
    ...
    if (numLayers > 4.0) {
        ...
        float2 p4 = ss_rot(rot4) * uv;
        float d4 = ss_centralMotif(p4, mix(2.2, 1.8, pat), sym4, breathe, 0.7, petalInfluence);
        float centralFade = smoothstep(mix(0.5, 0.48, pat), 0.12, r);
        float layer4alpha = min(numLayers - 4.0, 1.0);
        col += ss_glowLine(d4, hotGold * 0.7, coreGlow * 0.5, 0.0025, 0.022) * centralFade * layer4alpha;
    }
    ...
    if (numLayers > 5.0) {
        ...
        float pr0 = abs(ss_sdPolygon(ss_rot(t * rotSpeed * 0.02) * uv, rScale * 1.0, polySides1));
        float pr1 = abs(ss_sdPolygon(ss_rot(-t * rotSpeed * 0.025) * uv, rScale * 2.0, polySides2));
        float pr2 = abs(ss_sdPolygon(ss_rot(t * rotSpeed * 0.015) * uv, rScale * 3.0, 12.0));
        float pr3 = abs(ss_sdPolygon(ss_rot(-t * rotSpeed * 0.018) * uv, rScale * 4.0, polySides1));
        ringD = min(ringD, pr0);
        ringD = min(ringD, pr1);
        ringD = min(ringD, pr2);
        ringD = min(ringD, pr3);
        float layer5alpha = min(numLayers - 5.0, 1.0);
        col += ss_glowLine(ringD, brightGold * 0.5, dimGold * 0.5, 0.0012, 0.01) * layer5alpha;
    }

    // Golden spiral arms
    if (spiralInfluence > 0.01) {
        float spiral = ss_goldenSpiral(uv, t, rotSpeed);
        float spiralGlow = 0.015 / (spiral + 0.015);
        col += ss_gold(0.7 + t * 0.01) * spiralGlow * 0.25 * spiralInfluence;
    }
    ...
    if (pat > 0.3) {
        ...
        col += ss_gold(0.5 + t * 0.015) * outerGlow * 0.35 * decoFade;

        float outerRing2 = abs(r - objectRadius + 0.035) - 0.002;
        float outerGlow2 = 0.003 / (abs(outerRing2) + 0.003);
        col += ss_gold(0.6) * outerGlow2 * 0.2 * decoFade;

        float dotAngle = atan2(uv.y, uv.x);
        float dotSymmetry = mix(6.0, 12.0, pat);
        float dotA = (dotAngle + SS_PI / dotSymmetry) - (SS_TAU / dotSymmetry) * floor((dotAngle + SS_PI / dotSymmetry) / (SS_TAU / dotSymmetry)) - SS_PI / dotSymmetry;
